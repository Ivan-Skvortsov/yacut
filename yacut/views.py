...
def index_view():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        link = url_for('redirect_view', custom_id=url_map.short, _external=True)
        message = Markup(f'Ваша новая сслыка готова: <a href="{link}">{link}</a>')
        flash(message)
...
def redirect_view(custom_id):
    original_link = URL_map.query.filter_by(short=custom_id).first_or_404()
